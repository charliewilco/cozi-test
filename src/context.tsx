...
function removeSavedPet(state: IState, selected: IPet) {
  const mutableState = Object.assign({}, state);
  const petID = mutableState.saved.findIndex(pet => pet === selected);
...
function removePetFromFeed(state: IState, selected: IPet) {
  const mutableState = Object.assign({}, state);
  const petID = mutableState.pets.findIndex(pet => pet === selected);
...
function filterPets(state: IState): IState {
...
function getLocalPets(state: IState, locallySaved: IPet[]): IState {
...
const AppStateContext = React.createContext<[IState, IContextActions]>([
  {},
  {}
] as [IState, IContextActions]);
